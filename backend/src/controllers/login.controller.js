...
let createUser = (req, res) => {
    let user = req.body;
    let sql = "select email, password, role from account where email = ?";
    connection.query(sql, [user.email], (err, results) => {
        if (!err) {
            if (results.length <= 0) {
                let sql = "INSERT INTO `account`(name, email, password, role) VALUES (?, ?, ?, 'user')"
                connection.query(sql, [user.name, user.email, user.password], (err, results) => {
                    if (!err) {
...
let forgotPassword = (req, res) => {
    const user = req.body;
    let sql = "select email, password from account where email=? ";
    connection.query(sql, [user.email], (err, results) => {
        if (!err) {
            if (results.length <= 0) {
